function is_within_joint_limit = isWithinJointLimit(q, MTM_Name)

q = q(1:6);

json_file = 'JointLimitContraint.json';
...
if strcmp(MTM_Name, "MTML")
    joint_pos_upper_limit = config.MTML.joint_pos_upper_limit.'; 
    joint_pos_lower_limit = config.MTML.joint_pos_lower_limit.'; 
    coupling_index_list = {config.MTML.coupling_index_list.'};
    coupling_upper_limit = [config.MTML.coupling_upper_limit]; 
    coupling_lower_limit = [config.MTML.coupling_lower_limit];
elseif strcmp(MTM_Name, "MTMR")
    joint_pos_upper_limit = config.MTMR.joint_pos_upper_limit.'; 
    joint_pos_lower_limit = config.MTMR.joint_pos_lower_limit.'; 
    coupling_index_list = {config.MTMR.coupling_index_list.'};
    coupling_upper_limit = [config.MTMR.coupling_upper_limit]; 
    coupling_lower_limit = [config.MTMR.coupling_lower_limit];
else
    error("wrong MTM Name")
end

is_within_joint_limit = hw_joint_space_check(q, joint_pos_upper_limit, joint_pos_lower_limit,...
